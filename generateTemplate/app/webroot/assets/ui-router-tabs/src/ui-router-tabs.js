...
['$templateCache', function($templateCache) {
    var DEFAULT_TEMPLATE = '<div><tabset class="tab-container" type="{{type}}" vertical="{{vertical}}" ' +
      'justified="{{justified}}">' + '<tab ng-repeat="tab in tabs" heading="{{tab.heading | translate}}" ' +
    ...
    var SCROLLING_TEMPLATE = '<div scrolling-tabs-wrapper><tabset><tab class="crm-horizontal-tab" ng-repeat="tab in tabs" heading="{{tab.heading | translate}}" ' +
      'active="tab.active" disable="tab.disabled" ng-click="go(tab)"></tabset></div>' +
      '</tab>';

    $templateCache.put('ui-router-tabs-scrolling-template.html', SCROLLING_TEMPLATE);
}]
);
