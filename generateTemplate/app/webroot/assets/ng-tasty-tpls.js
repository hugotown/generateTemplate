 ...
 *
 */
 .directive('tastyHeadcol', function(){
  return {
    restrict: 'A',
    require: '^tastyTable',
    scope: {},
    link: function postLink(scope, element, attrs, tastyTable) {
      ...
      $(document).mousemove(function(e) {
        if(pressed) {
            console.log("here moving startWidth=[" + startWidth + "]" + 
                "e.pageX=[" + e.pageX + "]" +
                "startX=[" + startX + "]" +
                "newWidth=[" + (startWidth+(e.pageX-startX)) + "]" + 
                "pressed=[" + pressed + "]"
            );
            //console.log('width', start.width());
            //start.width(300);
            start.css('min-width', (startWidth+(e.pageX-startX)) + "px");
  ...
  return {
    ...
    link: function postLink(scope, element, attrs, tastyTable) {
      ...
      scope.setColumns = function () {
        ...
        //console.log('setColumns', scope.header);
        scope.header.columns.forEach(function (column, index) {
          ...
          sort = column.key;
          //console.log('setColumns', scope.header.sortBy + "===" + sort);
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('template/table/head.html',
    ...
    '    <span style="cursor: pointer;">{{column.name | translate}}</span>\n' +
...
module.run(['$templateCache', function($templateCache) {
  $templateCache.put('template/table/pagination.html',
    ...
    '    </ul>\n' +
    '  </div>\n' +
    '  <div class="col-xs-3 text-right" style="margin: 10px 0px; padding-right:25px;"><small>\n' +
    '    <p>{{"Page" | translate}} <span ng-bind="pagination.page"></span> \n' +
    '    {{"of" | translate}} <span ng-bind="pagination.pages"></span>,\n' +
    '    {{"of" | translate}} <span ng-bind="pagination.size"></span> {{"entries" | translate}}</p>\n' +
    '  </small></div>\n' +
    '</div>');
}]);
})();