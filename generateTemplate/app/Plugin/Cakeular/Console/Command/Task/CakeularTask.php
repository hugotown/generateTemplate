 ...
 */
class CakeularTask extends BakeTask {
 ...
 * @var array
 */
	public $scaffoldActions = array('html_index', 'html_list', 'html_add', 'html_edit', 'js_index', 'js_states', 'js_controller', 'js_route', 'js_service');
		...
		switch ($checkAction[0]) {
			...
			case 'js': {
				...
				elseif($action == "controller")
					{
						$filename = WWW_ROOT . DS . 'js'. DS . 'app' . DS . strtolower($this->controllerName) . DS . 'controllers' . DS . $this->_singularHumanName($this->controllerName). '.js';
					} elseif($action == "service")
					{
						$filename = WWW_ROOT . DS . 'js'. DS . 'app' . DS . strtolower($this->controllerName) . DS . 'services' . DS . $this->_singularHumanName($this->controllerName). '.js';
					} elseif($action == "route")
					{
						$filename = WWW_ROOT . DS . 'js'. DS . 'app' . DS . strtolower($this->controllerName) . DS . 'routes' . DS . $this->_singularHumanName($this->controllerName). '.js';
					} else{
						$filename = WWW_ROOT . DS . 'js' . DS . 'ui-statas' . DS . strtolower($this->controllerName) . '.js';	
					}
				break;
			}
			default:
