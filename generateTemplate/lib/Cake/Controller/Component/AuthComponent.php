 ...
 */
	public function startup(Controller $controller) {
		...
		if ($this->_isAllowed($controller)) {
			$this->log('Auth.startup::_isAllowed==true ctrl=' . $controller);
			return true;
		}
		$this->log('Auth.startup::before _getUser');
		if (!$this->_getUser()) {
			return $this->_unauthenticated($controller);
		}
		$this->log('Auth.startup::after _getUser');
 ...
 */
	protected function _getUser() {
		...
		//$this->log($this->_authenticateObjects);
		foreach ($this->_authenticateObjects as $auth) {
